...
本项目是一个支持递归嵌套、`custom: true`自定义选项、折叠/展开与选项概览、题型插件化扩展的互动问卷组件。无需后端即可静态部署，题目独立配置于 YAML 文件，UI 支持移动端自适应。
...
- 支持 `custom:true` 自动生成“其他”输入项，校验与展示全自动
- 未填写完必填项，“查看我的结果”按钮不可用
- 全部填写完后可一键复制结果（填写路径）
- 支持嵌套任意级子题
